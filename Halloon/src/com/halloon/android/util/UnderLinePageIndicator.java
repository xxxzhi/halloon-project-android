...
public class UnderLinePageIndicator extends View implements PageIndicator {
	...
	public void setLineWidthScale(float scale) {
		this.scale = scale;
	}
	...
	protected void onDraw(Canvas canvas) {
		...
		final float left = paddingLeft + pageWidth
				* (mCurrentPage + mPositionOffset) + exScale;
		...
		for (int i = 0; i < count; i++) {
			...
			canvas.drawText(til, 0, til.length(), paddingLeft + (pageWidth * i) + ((pageWidth - bounds.width()) / 2), exTop / 2 + (bounds.height() / 2), mPaintText);
		}
	}
		...
		case MotionEvent.ACTION_UP:
			if (!mIsDragging) {
				...
				final int pageWidth = (getWidth() - getPaddingLeft() - getPaddingRight())
						/ count;
				final int currentTouchPosition = (int) Math.floor(ev.getX()
						/ pageWidth);
